 ...
 */
Adafruit_Si7021::Adafruit_Si7021(unsigned int theHandle) {
...
bool Adafruit_Si7021::begin() {
    //Opens a connection to the sensor and tries to close it to see if it is working. 
  /*_wire->begin();
...
//TODO: Continue this function later
/*!
...
float Adafruit_Si7021::readHumidity() {
  ...
  i2cWriteByte(_handle, SI7021_MEASRH_NOHOLD_CMD);
...
void Adafruit_Si7021::reset() {
  //_wire->beginTransmission(_i2caddr);
  i2cWriteByte(_handle, SI7021_RESET_CMD);
...
void Adafruit_Si7021::_readRevision(void) {
    //_wire->beginTransmission(_i2caddr);
    i2cWriteByte(_handle, (uint8_t)(SI7021_FIRMVERS_CMD >> 8));
    i2cWriteByte(_handle, (uint8_t)(SI7021_FIRMVERS_CMD & 0xFF));
    //_wire->endTransmission();
    ...
    */
    ...
    uint8_t rev = i2cReadByte(_handle);
    i2cReadByte(_handle);
...
void Adafruit_Si7021::readSerialNumber() {
  //_wire->beginTransmission(_i2caddr);
  i2cWriteByte(_handle, (uint8_t)(SI7021_ID1_CMD >> 8));
  i2cWriteByte(_handle, (uint8_t)(SI7021_ID1_CMD & 0xFF));
  //_wire->endTransmission();

  sernum_a = i2cReadByte(_handle);
  i2cReadByte(_handle);
  sernum_a <<= 8;
  sernum_a |= i2cReadByte(_handle);
  i2cReadByte(_handle);
  ...
  //_wire->beginTransmission(_i2caddr);
  i2cWriteByte(_handle, (uint8_t)(SI7021_ID2_CMD >> 8));
  i2cWriteByte(_handle, (uint8_t)(SI7021_ID2_CMD & 0xFF));
  //_wire->endTransmission();

  sernum_b = i2cReadByte(_handle);
  i2cReadByte(_handle);
  sernum_b <<= 8;
  sernum_b |= i2cReadByte(_handle);
  i2cReadByte(_handle);
...
void Adafruit_Si7021::_writeRegister8(uint8_t reg, uint8_t value) {
  //_wire->beginTransmission(_i2caddr);
  // NOTE, might be able to get away with the following line instead
  //i2cWriteByteData(_handle, reg, value)
  i2cWriteByte(_handle, reg);
  i2cWriteByte(_handle, value);
  //_wire->endTransmission();
...
uint8_t Adafruit_Si7021::_readRegister8(uint8_t reg) {
  uint8_t value;
  //_wire->beginTransmission(_i2caddr);
  i2cWriteByte(_handle, (uint8_t)reg);
  //_wire->endTransmission();

    value = i2cReadByte(_handle);
    return value;
}
